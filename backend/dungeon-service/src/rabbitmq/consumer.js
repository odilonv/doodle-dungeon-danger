    ...
    try {
        ...
        channel.consume(DUNGEON_PROGRESS_QUEUE, async (message) => {
            if (message !== null) {
                ...
                console.log(` Received hero progression message: ${userId} - ${dungeonId}`);
...
export const consumeHeroFinishedDungeon = async () => {
    try {
        const connection = await amqp.connect(RABBITMQ_URL);
        const channel = await connection.createChannel();
        ...
        console.log(` Waiting for hero dungeon finish messages...`);
        channel.consume(DUNGEON_FINISHED_QUEUE, async (message) => {
            if (message !== null) {
                const { userId, dungeonId } = JSON.parse(message.content.toString());
                DungeonService.completeDungeonInstance(userId, dungeonId );
                channel.ack(message);
            }
        });
    } catch (error) {
